...
## Phase 4: コンテンツパックシステム・確率制御完了

全年齢版と18禁版でレア画像の排出率を制御するシステムを実装しました。

### Phase 4の実装内容

#### コンテンツパックシステム
- **モード制御**: 環境変数 `VITE_APP_MODE` で全年齢版/18禁版を切り替え
- **確率制御**: 18禁版のみ5-10%でレア画像を排出
- **データ駆動**: JSONファイルでコンテンツを管理
- **型安全性**: TypeScript strictモードで型安全性を確保

#### アセット解決システム
- **確率ベース選択**: `pickIllust()` 関数で確率に基づく画像選択
- **モード別パス**: 全年齢版/18禁版で異なる画像パスを生成
- **フォールバック**: エラー時の安全なフォールバック機能
- **テスト対応**: カスタムRNGでテスト可能な純粋関数

#### ガード機能
- **起動時検証**: 全年齢版でレア画像が設定されていないかチェック
- **強制フォールバック**: 不正な設定の場合は通常画像のみ使用
- **警告ログ**: 設定ミスをコンソールで警告

#### 将来拡張対応
- **カテゴリ追加**: 聴覚・触覚などのカテゴリをJSON追記のみで追加可能
- **ビルド分離**: 全年齢ビルドには18禁アセットを含めない
- **データ差し替え**: コンテンツパックの差し替えのみで運用可能
...
`.env` ファイルを作成してアプリケーションモードを設定してください：

```bash
# .env ファイル
VITE_APP_MODE=allages  # 全年齢版（デフォルト）
# VITE_APP_MODE=adult  # 18禁版
```

#### モード別の動作

- **全年齢版 (`allages`)**: レア画像は排出されません（確率0%）
- **18禁版 (`adult`)**: 8%の確率でレア画像を排出します
